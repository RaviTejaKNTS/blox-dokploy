...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: buildAlternates(`${BASE_PATH}/genres/${genre}/page/${pageNumber}`)
  };
}
...
export default async function GenreMusicIdsPaginatedPage({ params }: PageProps) {
  ...
  const { songs, total, totalPages } = await loadGenreMusicIdsPageData(pageNumber, genre.label);
  ...
  const breadcrumbNavItems = [
    ...
    { label: genre.label, href: `${BASE_PATH}/genres/${genre.slug}` },
  ...
  const breadcrumbSchema = JSON.stringify(
    breadcrumbJsonLd([
      ...
      { name: genre.label, url: `${SITE_URL.replace(/\/$/, "")}${BASE_PATH}/genres/${genre.slug}` },
