...
export function RichMarkdownEditor({ label, value, onChange, placeholder, height }: RichMarkdownEditorProps) {
  ...
  return (
    <div className="space-y-3" data-placeholder-enhanced="true">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <label className="text-sm font-semibold text-foreground">{label}</label>
        <div className="flex flex-wrap items-center gap-3">
          {mode === "rich" && activePlaceholder ? (
            <div className="flex items-center gap-2 rounded-full border border-border/60 bg-background/60 px-3 py-1 text-xs font-semibold text-muted">
              ...
              <button
                ...
                onClick={editActivePlaceholder}
              >
                Edit label
              </button>
              <button
                type="button"
                className="text-destructive transition hover:underline"
                onClick={removeActivePlaceholder}
              >
                Remove
              </button>
            </div>
          ) : null}
          <div className="flex items-center gap-1 rounded-full border border-border/60 bg-background/60 p-1 text-xs font-semibold text-muted">
            <button
              type="button"
              className={`rounded-full px-3 py-1 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                mode === "rich" ? "bg-accent/10 text-accent shadow-sm" : "hover:text-foreground"
              }`}
            ...
            <button
              type="button"
              className={`rounded-full px-3 py-1 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                mode === "markdown" ? "bg-accent/10 text-accent shadow-sm" : "hover:text-foreground"
              }`}
      ...
      <div className="space-y-2 rounded-2xl border border-border/60 bg-surface p-4 shadow-inner">
        {mode === "rich" ? (
        ...
        {mode === "markdown" ? (
          <textarea
            ...
            spellCheck
            className="w-full resize-vertical rounded-2xl border border-border/60 bg-surface p-4 font-mono text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-accent"
