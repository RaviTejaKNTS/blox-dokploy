...
export function ArticleEditorForm({ article, authors, categories }: ArticleEditorFormProps) {
  ...
  const [activeTab, setActiveTab] = useState<"content" | "meta" | "media">("content");
  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-surface/80 px-6 py-4 shadow-soft">
        <div className="flex flex-col gap-1 text-sm">
          <button
            type="button"
            onClick={handleBackClick}
            className="self-start text-xs font-semibold uppercase tracking-wide text-muted underline-offset-4 hover:underline"
          >
            ← Back to articles
          </button>
          <h1 className="text-xl font-semibold text-foreground">{article ? `Edit ${article.title}` : "Create article"}</h1>
          <p className="text-xs text-muted">{lastUpdatedLabel}</p>
        </div>
      ...
      <div className="flex gap-3 text-sm">
        ...
        <button
          ...
          className={`rounded-lg px-3 py-2 font-semibold ${activeTab === "media" ? "bg-accent text-white" : "bg-surface text-muted"}`}
        >
          Media
        </button>
      </div>

      <form id="article-editor-form" onSubmit={onSubmit} className="space-y-8">
        {activeTab === "content" ? (
          <section className="space-y-8">
            <div className="grid gap-4 rounded-2xl border border-border/60 bg-surface/80 p-6 shadow-soft">
              ...
              <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_200px] md:items-end md:gap-3">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted">
                    Used for URLs and cover filenames. Leave empty to auto-generate from the title.
                  </p>
                </div>
                {liveUrl ? (
                  <Link
                    ...
                    className="inline-flex items-center justify-center rounded-lg border border-border/60 bg-background px-3 py-2 text-xs font-semibold text-muted transition hover:border-border/30 hover:text-foreground"
                  >
                    View live page ↗
                  </Link>
                ) : null}
            ...
            <div className="grid gap-4 rounded-2xl border border-border/60 bg-surface/80 p-6 shadow-soft">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2 text-xs text-muted">
                  <span className="rounded-full bg-surface-muted px-2 py-1">{wordCount} words</span>
                </div>
              </div>
              <label
                ...
                className={`block rounded-lg border border-dashed border-border/60 bg-background/60 px-4 py-3 text-xs text-muted transition hover:border-accent ${
                  isMarkdownDragging ? "border-accent bg-accent/10 text-accent" : ""
                }`}
              >
                <input id="article-markdown-upload" type="file" accept="text/markdown,.md" onChange={onMarkdownInputChange} className="hidden" />
                <span className="font-semibold text-foreground">Drop a markdown file</span> or click to replace the content.
                {markdownError ? <span className="ml-2 text-destructive">{markdownError}</span> : null}
              </label>
              <RichMarkdownEditor
                label="Content"
                value={contentValue}
            ...
            <div className="space-y-4 rounded-2xl border border-border/60 bg-surface/80 p-6 shadow-soft">
              ...
              {coverValue ? (
                <div className="overflow-hidden rounded-xl border border-border/60 bg-background/60">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={coverValue} alt="Article cover" className="h-48 w-full object-cover" />
                </div>
              ) : null}
            </div>
        ...
        {activeTab === "media" ? (
          <section className="space-y-6 rounded-2xl border border-border/60 bg-surface/80 p-6 shadow-soft">
            {slugValue ? (
      ...
      <div className="flex justify-end">
        <button
          ...
          form="article-editor-form"
          disabled={isPending}
          className="rounded-lg bg-accent px-6 py-3 text-sm font-semibold text-white transition hover:bg-accent-dark disabled:cursor-not-allowed disabled:opacity-60"
        >
