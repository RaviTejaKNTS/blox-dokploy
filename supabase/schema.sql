...
create table if not exists public.revalidation_events (
  id uuid primary key default uuid_generate_v4(),
  entity_type text not null check (entity_type in ('code','article','list','author','event','checklist','tool','catalog','music','quiz')),
  slug text not null,
  source text,
  created_at timestamptz not null default now()
);
...
create trigger trg_user_quiz_progress_updated_at before update on public.user_quiz_progress
for each row execute function public.set_updated_at();
...
-- quiz pages table for game quizzes
