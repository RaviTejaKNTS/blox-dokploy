-- Add primary metric config to game_lists and surface it in game_lists_view entries

alter table if exists public.game_lists
  add column if not exists primary_metric_key text,
  add column if not exists primary_metric_label text;
  ...
  coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'extra', e.extra,
        'game_id', e.game_id,
        'metric_key', coalesce(e.extra->>'metric', l.primary_metric_key),
        'metric_label', coalesce(e.extra->>'metric_label', l.primary_metric_label),
