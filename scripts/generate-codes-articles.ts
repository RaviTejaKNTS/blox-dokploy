...
const normalizeCodesSlug = (input?: string | null, fallback?: string | null): string =>
  stripCodesSuffix(normalizeGameSlug(input, fallback));
...
async function main() {
  ...
  let slug = normalizeCodesSlug(canonicalName, gameName);
  ...
  slug = normalizeCodesSlug(name, slug);
  ...
  if (derivedUniverseSlug) {
    finalSlug = normalizeCodesSlug(derivedUniverseSlug, slug);
  }
